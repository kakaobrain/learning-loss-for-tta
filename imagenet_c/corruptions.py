...
def glass_blur(x, severity=1, size=32):
...
def fog(x, severity=1):
    if x.width > CIFAR_WH:
        if max(x.width, x.height) > 1024:
            mapsize = 2048
        elif max(x.width, x.height) > 512:
            mapsize = 1024
        elif max(x.width, x.height) > 256:
            mapsize = 512
        else:
            mapsize = 256
...
def frost(x, severity=1):
    if x.width > CIFAR_WH:
        ...
        need_resize = False
        if x.width > 224 or x.height > 224:
            need_resize = True
            fxy = max(x.width, x.height) / 224.
    else:
    ...
    if need_resize:
        frost = cv2.resize(frost, (0, 0), fx=fxy, fy=fxy)
    # randomly crop and convert to rgb
    x_start, y_start = np.random.randint(0, frost.shape[1] - w), np.random.randint(0, frost.shape[0] - h)
    frost = frost[y_start:y_start + h, x_start:x_start + w][..., [2, 1, 0]]
